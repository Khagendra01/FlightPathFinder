...
public class DijkstraSP {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		CsvReader myModel = new CsvReader();
		...
		System.out.println("Enter the origins.");
		String origins = inputScanner.nextLine();

		ArrayList<String> origin = individualizeTheMultiInput(origins.toCharArray());

		while (keepGoing) {
			System.out.println("Enter the destination.");
			String destination = inputScanner.nextLine();

			int destinationValue = myModel.myMap.get(destination);

			for (String eachOrigin : origin) {
				DijkstraSP sp = new DijkstraSP(myModel.thisGraph, myModel.myMap.get(eachOrigin));
				if (sp.hasPathTo(destinationValue)) {
					if (sp.distTo(destinationValue) < minCost) {
						minCostPath = sp.pathTo(destinationValue);
						minCost = sp.distTo(destinationValue);
					}
				}
			}

			if (minCost == Double.MAX_VALUE) {
				System.out.println("No Path Exist from " + origins + " to " + destination);
			} else {

				resultBuilder = "Fare: $" + minCost;

				int edgeSplitter = 1;
				for (DirectedEdge e : minCostPath) {
					if (edgeSplitter % 2 == 1) {
						resultBuilder = myModel.myMapReverse.get(e.to()) + " $" + e.weight() + " /" + resultBuilder;
					} else {
						resultBuilder = myModel.myMapReverse.get(e.to()) + " " + myModel.myMapReverse.get(e.from())
								+ "-" + resultBuilder;
					}
					edgeSplitter++;
				}
				System.out.println(resultBuilder);
				minCost = Double.MAX_VALUE;// reset the minCost to make new comparision for new input of the destination
			}
		}

	}

	private static ArrayList<String> individualizeTheMultiInput(char[] origins) {
		ArrayList<String> origin = new ArrayList<>();
		String stringBuilder = "";
		for (char eachChar : origins) {
			if (eachChar == ' ') {
				origin.add(stringBuilder);
				stringBuilder = "";
			} else {
				stringBuilder = stringBuilder + eachChar;
			}
		}
		origin.add(stringBuilder);
		return origin;
